...
async function deleteInventory(inventoryId: string) {
  ...
  if (!response?.ok) {
    toast({
      ...
      description: "Your inventory was not deleted. Please try again.",
      variant: "destructive",
    });
  }
...
interface InventoryOperationsProps {
  inventory: Pick<Inventory, "id" | "location">;
}

export function InventoryOperations({ inventory }: InventoryOperationsProps) {
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async (event) => {
                ...
                const deleted = await deleteInventory(inventory.id);
