...
async function deleteWarehouse(warehouseId: string) {
  ...
  if (!response?.ok) {
    toast({
      ...
      description: "Your warehouse was not deleted. Please try again.",
      variant: "destructive",
    });
  }
...
export function WarehouseOperations({ warehouse }: WarehouseOperationsProps) {
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async (event) => {
                ...
                const deleted = await deleteWarehouse(warehouse.id);
